        ...
        public void Execute()
        {
            for (int i = 0; i < TriggerEntitiesCount[0]; i++)
            {
                ...
                if (!OverlappingGroup.Exists(overlappingEntity))
                {
                    ...
                    {
                        case TriggerVolumeType.Enemy:
                            CommandBuffer.AddComponent<EnemyOverlappingTriggerVolume>(overlappingEntity, new EnemyOverlappingTriggerVolume());
                            break;
        ...
        public void Execute()
        {
            for (int index = 0; index < OverlappingEntities.Length; index++)
            {
                ...
                if (overlappingFrame != volumeFrame)
                {
                    ...
                    {
                        case TriggerVolumeType.Enemy:
                            CommandBuffer.RemoveComponent<EnemyOverlappingTriggerVolume>(entity);
                            break;
                        case TriggerVolumeType.None:
                        default:
                            CommandBuffer.RemoveComponent<EnemyOverlappingTriggerVolume>(entity);
